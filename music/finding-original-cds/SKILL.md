...
- A standard **UPC barcode** on the back argues against a club edition, but **weakly — do not clear a copy on the barcode alone.** Verified 2026-08-21: a Columbia House *Disintegration* was indexed on eBay under the original's UPC `075596085526`, and a BMG copy carried `D 101109` while being advertised as `9 60855-2`. **The decisive tell is the licence line printed on the tray card** — "MANUFACTURED BY COLUMBIA HOUSE UNDER LICENSE", or "BMG Direct Marketing, Inc. under License" with an Indianapolis address. Read the tray, not the barcode.
- **The plant credit on the disc face is not the pressing.** *Disintegration* release `178726` is an SRC pressing whose disc reads "Made In U.S.A. By WEA Manufacturing Inc." — `SRC` appears only in the matrix runout. A photo of the disc face can never distinguish pressing plants; only the runout can.
