...
While optimized for product/platform teams (DevTools, Data Platform, Developer Infrastructure, etc.), these principles apply broadly to engineering, product, design, and operations teams.
...
- **wiki**: Search your internal wiki. Use this to:
...
- **docs**: Search public product documentation. Use this to:
...
- **gitlab**: Search your internal code repository. Use this to:
  - Verify that referenced codebases, file paths, and struct names actually exist (e.g., confirm `api/job.go` contains a `JobRequest` struct).
  - Check whether proposed API changes align with the current implementation.
  - Look up existing merge requests or recent changes related to the PRD's scope.
  - Understand the current state of the codebase when the PRD proposes modifications to existing services.

When drafting or reviewing a PRD, **start by searching the wiki** for related work to avoid duplicating efforts or missing important context. When the PRD references specific codebases or implementation details, **use GitLab to verify** that those references are accurate.

---

## **1.2 Skills to Apply**

Read and apply relevant skills from `.opencode/skills/` based on the PRD context:

| Skill | When to Use |
|-------|-------------|
| `pm-thinking` | **Always** — ensures PRDs are problem-first, outcome-focused, and measurable |
| `company-context` | When drafting/reviewing PRDs for your products — grounds requirements in your product context |
| `team-context` | When drafting/reviewing PRDs for a specific team — applies team-specific principles and constraints |

**Apply skills proactively** — load relevant context before drafting or reviewing any PRD.
...
#### Good Examples
- "**Primary:** Backend engineers deploying new services (200+ engineers across 15+ teams). **Secondary:** SREs who support these services in production."
...
- "**Primary:** New engineers in their first 90 days (50-75 new hires per quarter). **Secondary:** Engineering managers and onboarding buddies who support new hires."
...
See `03-reference/confluence-wiki-markup-syntax.md` for complete syntax reference. Key points:
- **Headers:** `h1.`, `h2.`, `h3.` (not Markdown `#`, `##`, `###`)
- **Bold:** `*bold text*` (not `**bold**`)
- **Tables:** `||` for headers, `|` for cells, `\\` for line breaks within cells
- **Lists:** NO extra line breaks between list items (`*` for bullets, `#` for numbered)
